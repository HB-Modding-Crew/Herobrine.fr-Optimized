...
def copy_dir_to_zip(from_path, to_path, archive_path):
    files = []

    with zipfile.ZipFile(archive_path) as archive:
        for zipinfo in archive.infolist():
            if not zipinfo.filename.startswith(to_path):
                files.append((zipinfo.filename, archive.read(zipinfo.filename)))
    ...
    with zipfile.ZipFile(archive_path, "w") as archive:
        ...
        for root, dirs, files in os.walk(from_path):
            for file in files:
                archive.write(os.path.join(root, file), to_path + "/" + os.path.join(root, file)[len(from_path) + 1:])
...
def copy_dir_from_zip(from_path, to_path, archive_path):
    with zipfile.ZipFile(archive_path) as archive:
        for zipinfo in archive.infolist():
            if zipinfo.filename.startswith(from_path):
                # If the path is a directory, create it
                if zipinfo.filename.endswith("/"):
                    os.makedirs(os.path.join(to_path, zipinfo.filename[len(from_path) + 1:]), exist_ok=True)
                else:
                    with open(os.path.join(to_path, zipinfo.filename[len(from_path) + 1:]), "wb") as file:
                        file.write(archive.read(zipinfo.filename))

def remove_from_zip(paths, archive_path):
    files = []

    with zipfile.ZipFile(archive_path) as archive:
        for zipinfo in archive.infolist():
            if zipinfo.filename not in paths:
                files.append((zipinfo.filename, archive.read(zipinfo.filename)))
...
def remove_dir_from_zip(dir_path, archive_path):
    files = []

    with zipfile.ZipFile(archive_path) as archive:
        for zipinfo in archive.infolist():
            if not zipinfo.filename.startswith(dir_path):
                files.append((zipinfo.filename, archive.read(zipinfo.filename)))
